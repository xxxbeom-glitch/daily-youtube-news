...
export async function listUploadedVideoIds(handle, token, start, end) {
  const channel = await getUploadsPlaylistId(handle, token);
  if (!channel.uploadsPlaylistId) {
    throw new Error(`No uploads playlist found for ${handle}`);
  }
  ...
  do {
    ...
    for (const item of data.items || []) {
      ...
      if (!publishedRaw || !videoId) continue;
    ...
    if (crossedStart || pageCount >= 10) break;
...
export async function findPlaylistByTitle(token, title) {
  ...
  do {
    ...
    const found = (data.items || []).find((item) => item.snippet?.title === title);
