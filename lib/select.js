...
function heuristicScore(video) {
  ...
  let score = 0;
  for (const term of STRONG_INCLUDE) {
    if (text.includes(term.toLowerCase())) score += 2;
  }
  for (const term of POLITICS_ONLY) {
...
- Government policy when it materially affects everyday life, the economy, health, housing, labor, taxes, insurance, safety, or transport.
...
- Sports.
- Minor local incidents or low-impact briefs.

SELECTION RULES:
- Pick 10 to 15 videos when enough relevant material exists; target 15.
- Never pad with entertainment, sports, or routine partisan politics just to hit the minimum.
...
- Prefer a clear, information-dense 2-5 minute report and higher-impact stories.
...
- score is an integer 0-100.`;
  ...
  try {
    ...
    for (const item of parsed.selected || []) {
      ...
      if (selectedIds.length >= 15) break;
    }

    const videos = selectedIds.map((id) => byId.get(id));
    if (videos.length === 0) throw new Error("Model selection contained no valid candidate video IDs");

    return { mode: `openai:${model}`, videos };
  } catch (error) {
