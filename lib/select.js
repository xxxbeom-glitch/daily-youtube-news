...
const CATEGORY_RULES = [
  { category: "국제", terms: ["전쟁", "공습", "폭격", "휴전", "미사일", "핵", "테러", "호르무즈", "이란", "이스라엘", "우크라이나", "러시아", "제재", "군사", "분쟁", "미중", "외교", "안보"] },
  { category: "경제", terms: ["경제", "금리", "환율", "물가", "증시", "코스피", "코스닥", "부동산", "전세", "주택", "관세", "무역", "수출", "수입", "고용", "실업", "임금", "기업", "산업", "반도체"] },
  { category: "사건·재난", terms: ["화재", "산불", "지진", "홍수", "태풍", "폭우", "붕괴", "폭발", "참사", "사망", "실종", "사고", "범죄", "살인", "납치", "재난", "수사"] },
  { category: "건강", terms: ["감염", "전염병", "독감", "코로나", "질병", "백신", "보건", "건강보험", "의료", "병원"] },
  { category: "정책", terms: ["정책", "세금", "보험", "연금", "교육", "노동", "전기요금", "가스요금", "정부", "법안", "제도"] },
  { category: "생활", terms: ["생활", "소비자", "생활비", "식품", "먹거리", "유통", "택배", "환경", "날씨", "여행", "교통", "주거"] },
];
...
function inferCategory(video) {
  const text = `${video.title} ${video.description}`.toLowerCase();
  for (const rule of CATEGORY_RULES) {
    if (rule.terms.some((term) => text.includes(term.toLowerCase()))) return rule.category;
  }
  return "사회";
}

function normalizeCategory(value, video) {
  const text = String(value || "").trim().toLowerCase();
  if (/국제|외교|안보|전쟁/.test(text)) return "국제";
  if (/경제|금융|산업|부동산/.test(text)) return "경제";
  if (/사건|사고|재난|범죄|안전/.test(text)) return "사건·재난";
  if (/건강|의료|보건|감염/.test(text)) return "건강";
  if (/정책|정부|제도|교육|노동/.test(text)) return "정책";
  if (/생활|소비|환경|교통|여행|식품/.test(text)) return "생활";
  if (/사회/.test(text)) return "사회";
  return inferCategory(video);
}
...
function withSelectionMeta(video, { category, score, reason } = {}) {
  return {
    ...video,
    selectionCategory: normalizeCategory(category, video),
    selectionScore: clampScore(score),
    selectionReason: typeof reason === "string" ? reason.slice(0, 300) : "",
  };
}

function orderByImportanceWithDiversity(videos) {
  const remaining = [...videos].sort((a, b) =>
  ...
  while (remaining.length) {
    ...
    if (ordered.length >= 2) {
      const last = ordered[ordered.length - 1]?.selectionCategory;
      const previous = ordered[ordered.length - 2]?.selectionCategory;
      if (last && last === previous) blockedCategory = last;
    }

    let index = 0;
    if (blockedCategory) {
      const alternative = remaining.findIndex((video) => video.selectionCategory !== blockedCategory);
...
function heuristicSelection(candidates) {
  ...
  for (const item of ranked) {
    ...
    selected.push(withSelectionMeta(item.video, {
      category: inferCategory(item.video),
      score: 50 + item.score * 5,
  ...
  return {
    ...
    videos: orderByImportanceWithDiversity(selected),
  };
}
...
- A politician may appear if the story is really about economic policy, war/security, sanctions, tariffs, public health, or another included substantive issue.
...
- Routine culture/arts features are not a target category. Only include a culture-related story when it has clear broader social, economic, safety, or policy significance.

SELECTION RULES:
- Select between ${MIN_SELECTION} and ${MAX_SELECTION} videos when at least ${MIN_SELECTION} eligible candidates exist; target about 9-10.
...
- Prefer clear, information-dense 2-5 minute reports and higher-impact stories, while maintaining a useful mix across society, economy, international affairs, incidents, health, policy, and everyday life.
- For every selected video, assign category as exactly one of: 국제, 경제, 사회, 사건·재난, 건강, 정책, 생활.
- score is IMPORTANCE for today's viewer, not confidence. Use an integer 0-100. Major national/international developments should score higher than useful but lower-impact lifestyle items.
- Return ONLY valid JSON with this exact shape: {"selected":[{"videoId":"...","category":"경제","reason":"...","score":90}]}.
- Do not try to arrange the final playlist order; the application will sort by score and diversify categories.`;
  ...
  try {
    ...
    for (const item of parsed.selected || []) {
      ...
      if (!video || selectedIds.includes(item.videoId)) continue;
    ...
    if (selectedIds.length < MIN_SELECTION && candidates.length >= MIN_SELECTION) {
      ...
      for (const video of fallback.videos) {
        ...
        metadataById.set(video.videoId, {
          category: video.selectionCategory,
          score: video.selectionScore,
          reason: video.selectionReason,
        });
        supplemented = true;
    ...
    const videos = selectedIds
      ...
      .map((id) => withSelectionMeta(byId.get(id), metadataById.get(id)));

    return {
      mode: supplemented ? `openai:${model}+supplement` : `openai:${model}`,
      videos: orderByImportanceWithDiversity(videos),
    };
  } catch (error) {
    const fallback = heuristicSelection(candidates);
    return {
