...
export function getNewsWindow(now = new Date()) {
  ...
  // 08:00 KST is 23:00 UTC on the previous calendar day.
  let endMs = Date.UTC(year, month - 1, day, -1, 0, 0, 0);
  if (now.getTime() < endMs) endMs -= DAY;

  // Collect from the previous day at 18:00 KST through 08:00 KST.
  const startMs = endMs - 14 * HOUR;
